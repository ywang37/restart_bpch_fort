...
! 
      ...
      IF ( ABS(angle_center_arg) > 1.00001) 
...
!  arguments
      ...
      REAL*8, INTENT(OUT) :: ZGRID(ILAYERS+1)
      REAL*8, INTENT(OUT) :: PGRID(ILAYERS+1)
...
!  local variables
      ...
      DO K = 2, IEDGES
         ZGRID(K) = 7.3*LOG( 1013. /
     &              ( (PS - 1.)*( REAL(K-1)/REAL(ILAYERS) ) ) )
...
!------------------------------------------------------------------------------
!
      REAL*8 FUNCTION SCATTERING_ANGLE(THETA_SUN, THETA_SAT,
     &                                 AZIMUTH) 

      !USE INDEX_MOD,         ONLY: DPIE
      ! Arguments
